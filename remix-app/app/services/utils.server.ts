 ...
 * ```
 */
export type WrappedFetchResponse<T> = {
  success: true;
  message: string;
  data: T;
};
...
): Promise<WrappedFetchResponse<T> | ErrorStack> {
  try {
    ...
    if (error instanceof Error) {
      ...
      return { error: message, stackError: error.stack };
    }
...
export function mockFetchResponse<T>(
  ...
  message: string,
  data?: T
): WrappedFetchResponse<T> | ErrorStack {
  if (type === 'Fail') {
